...
import { storeOwnerData } from '../../formData';
import InputFields from '../../components/inputfields';
import { Card, CardBody, Row, Col, Button, Form } from 'reactstrap';

class EditStoreOwnerForm extends Component {
	constructor(props) {
		super(props);
		this.state = {
			value: null
	...
	handleSave() {
		this.props.handleFormSave(
			{
				newStoreOwner: this.storeOwnerId.getValue()
			});
		this.props.handleFormOpen();
	}
	getAllFields(selectedItem) {
		return storeOwnerData.map((item, index) => {
			return (
			...
			<Col md="12">
				<Card>
					<CardBody className="ml-4 mr-4 text-center justify-content-center">
						<span className="font-xl font-weight-bold mb-5">Create New Store Owner</span>
...
export default EditStoreOwnerForm;